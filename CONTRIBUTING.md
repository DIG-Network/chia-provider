# Contributing to @dignetwork/chia-provider
...
- Include crate version (`Cargo.toml` `version`, or `cargo tree -p @dignetwork/chia-provider`) and
...
4. **Bump the version.** Every PR that changes behaviour, fixes a bug, or otherwise
   warrants a release bumps `version` in `package.json` as the **last commit before
   merge**, following SemVer:
   ...
   - **major** — a breaking change (removed/renamed API or a changed default).

   A CI gate fails any PR whose version does not increase over `main`.
